function [btime, burst, binary_events] = detect_burst_timeseries(Fs, times, signal, low, high, thrA_factor)
    ...
    % 3. binary_evernts   [1 x N vector]          : binary time array with 1 indicating burst events and 0 otherwise

    % Written by SungJun Cho, April 3, 2021
    % Last Modified on November 12, 2023
    % Adapted from https://github.com/jeelabKIST/Cho2022_BurstDetection/blob/main/utils/util_burst_detection/detect_burst_timeseries.m
    %% Set Parameters
    ...
    %% Get Binary Time Points Marking Burst Events
    binary_events = zeros(size(times));
    for i = 1:length(btime)
        start_idx = find(times == btime{i}(1));
        end_idx = find(times == btime{i}(end));
        binary_events(start_idx:end_idx) = 1;
    end
end